...
SAM_Feature_Fields = [
    ...
    QgsField("PATCH_ID", QMetaType.Int),
    QgsField("GROUP_ID", QMetaType.QString),
    QgsField("GROUP_COUNT", QMetaType.Int),
    QgsField("AREA", QMetaType.Double),
    QgsField("N_FG", QMetaType.Int),
    QgsField("N_BG", QMetaType.Int),
    QgsField("IS_BOX", QMetaType.Bool),
...
class RectangleMapTool(QgsMapToolEmitPoint):
    ...
    def __init__(
        ...
        # New: right-click save callback
        self.right_click_save = None
    ...
    def canvasPressEvent(self, e):
        ...
        if e.button() == Qt.RightButton:
            if hasattr(self, 'right_click_save') and callable(self.right_click_save):
                if self.right_click_save():
                    return  # If right-click save is handled, return directly
...
class SAM_PolygonFeature:
    ...
    def __init__(
        ...
        default_name: str = "polygon_sam2_mask",
        kwargs_preview_polygon: Dict = {},
        kwargs_prompt_polygon: Dict = {},
        overwrite: bool = False,
        skip_init: bool = False,  # New parameter
    ):
        self.qgis_project = QgsProject.instance()
        self.img_crs_manager = img_crs_manager
        ...
        if skip_init:
            # Only set basic attributes, do not initialize layer
            # self.layer = None
            MessageTool.MessageLog("SAM_PolygonFeature initialized without loading or creating a layer.")
            canvas_crs = self.canvas.mapSettings().destinationCrs()
            self.layer = QgsVectorLayer("Polygon?crs=" + canvas_crs.authid(), self.default_name, "memory")
            # self.layer = QgsVectorLayer("Polygon", self.default_name, "memory")
            # Use the image's own CRS to avoid dynamic reprojection
    ...
    def _init_layer(
        ...
        if layer_list:
            ...
            MessageTool.MessageLog(
                f"A temporal layer: '{self.layer_name}' is created, please remember to save it before quit.")
    ...
    def add_geojson_feature_to_layer(
        ...
        for idx, geom in enumerate(geojson):
            ...
            patch_id = geom["properties"]["patch_id"]
            score = geom["properties"]["score"]
            score = round(float(score), 2)
            if "mode" in geom["properties"]:
                if geom["properties"]["mode"] == "box_predictor":
                    group_id = patch_id
        ...
        idx_n_gm = self.layer.fields().indexOf("GROUNP_COUNT")
        if idx_n_gm > -1:
